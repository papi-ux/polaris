...
namespace nvenc {
  ...
  bool nvenc_base::create_encoder(const nvenc_config &config, const video::config_t &client_config, const nvenc_colorspace_t &colorspace, NV_ENC_BUFFER_FORMAT buffer_format) {
    ...
    // Save the initialization parameters for runtime reconfiguration (e.g., adaptive bitrate)
...
#undef nvenc_status_case
  ...
  bool nvenc_base::update_bitrate(int new_bitrate_kbps) {
    if (!encoder || !nvenc) {
      return false;
    }

    auto new_bitrate_bps = new_bitrate_kbps * 1000;
    if (new_bitrate_bps == static_cast<int>(saved_enc_config.rcParams.averageBitRate)) {
      return true;  // No change needed
    }

    // Update the saved config with the new bitrate
    saved_enc_config.rcParams.averageBitRate = new_bitrate_bps;
    ...
    reconfig_params.version = min_struct_version(NV_ENC_RECONFIGURE_PARAMS_VER);
    reconfig_params.reInitEncodeParams = saved_init_params;
    reconfig_params.reInitEncodeParams.encodeConfig = &saved_enc_config;
    reconfig_params.forceIDR = 1;
    reconfig_params.resetEncoder = 1;

    if (nvenc_failed(nvenc->nvEncReconfigureEncoder(encoder, &reconfig_params))) {
      BOOST_LOG(error) << "NvEnc: NvEncReconfigureEncoder() failed: " << last_nvenc_error_string;
      return false;
    }

    BOOST_LOG(debug) << "NvEnc: bitrate reconfigured to " << new_bitrate_kbps << " kbps";
    return true;
  }
