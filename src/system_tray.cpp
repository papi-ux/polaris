...
namespace system_tray {
  ...
  void tray_quit_cb([[maybe_unused]] struct tray_menu *item) {
    ...
    if (GetConsoleWindow() == nullptr) {
      lifetime::exit_sunshine(ERROR_SHUTDOWN_IN_PROGRESS, true, "quit selected in the system tray");
      return;
    }
  #endif

    lifetime::exit_sunshine(0, true, "quit selected in the system tray");
  }
  ...
  static struct tray tray = {
    ...
    .menu =
      (struct tray_menu[]) {
        ...
        { .text = "Open Polaris", .cb = tray_open_ui_cb },
        { .text = "-" },
        // { .text = "-" },
        // { .text = "Donate",
        //   .submenu =
  ...
  #ifdef _WIN32
    std::string tmp_str = "Open Polaris (" + config::nvhttp.sunshine_name + ":" + std::to_string(net::map_port(confighttp::PORT_HTTPS)) + ")";
    static const std::string title_str = utf8ToAcp(tmp_str);
  #else
    static const std::string title_str = "Open Polaris (" + config::nvhttp.sunshine_name + ":" + std::to_string(net::map_port(confighttp::PORT_HTTPS)) + ")";
  #endif
