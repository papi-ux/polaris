...
namespace platf::dxgi {
  ...
  inline platf::frame_format_e frame_format_from_dxgi_format(DXGI_FORMAT format) {
    switch (format) {
      case DXGI_FORMAT_R16G16B16A16_FLOAT:
        return platf::frame_format_e::rgba16f;
  ...
  inline platf::frame_metadata_t frame_metadata_from_capture(DXGI_FORMAT format, platf::frame_transport_e transport, platf::frame_residency_e residency) {
    const auto resolved_format = format == DXGI_FORMAT_UNKNOWN ? DXGI_FORMAT_B8G8R8A8_UNORM : format;
    return {
      .transport = transport,
      .residency = residency,
      .format = frame_format_from_dxgi_format(resolved_format),
    };
  }
  ...
  public:
    ...
    DXGI_RATIONAL client_frame_rate_strict {0, 1};
