...
#include "src/video.h"
  ...
  #define POLARIS_SHADERS_DIR POLARIS_ASSETS_DIR "/shaders/directx"
#endif
namespace platf {
...
namespace platf::dxgi {
  ...
  public:
    ID3D11Texture2D *output_resource() const {
...
#undef create_pixel_shader_helper
  ...
  public:
    ...
    mem_type_e native_output_device_type() const override {
      return mem_type_e::dxgi;
    }

    void *native_output_resource() const override {
      return base.output_resource();
    }
  ...
  capture_e display_ddup_vram_t::snapshot(const pull_free_image_cb_t &pull_free_image_cb, std::shared_ptr<platf::img_t> &img_out, std::chrono::milliseconds timeout, bool cursor_visible) {
    ...
    if (img_out) {
      img_out->frame_metadata = frame_metadata_from_capture(capture_format, platf::frame_transport_e::dxgi_duplication, platf::frame_residency_e::gpu);
  ...
  capture_e display_wgc_vram_t::snapshot(const pull_free_image_cb_t &pull_free_image_cb, std::shared_ptr<platf::img_t> &img_out, std::chrono::milliseconds timeout, bool cursor_visible) {
    ...
    img_out = img;
    if (img_out) {
      img_out->frame_metadata = frame_metadata_from_capture(capture_format, platf::frame_transport_e::wgc, platf::frame_residency_e::gpu);
      img_out->frame_timestamp = frame_timestamp;
      stream_stats::update_capture_metadata(img_out->frame_metadata);
    }
  ...
  int display_vram_t::complete_img(platf::img_t *img_base, bool dummy) {
    ...
    img->data = nullptr;
    img->native_device_type = mem_type_e::unknown;
    img->native_resource = nullptr;
    img->native_fd = -1;
    ...
    img->native_device_type = mem_type_e::dxgi;
    img->native_resource = img->capture_texture.get();
    img->native_fd = -1;
...
#define compile_vertex_shader_helper(x) \
  if (!(x##_hlsl = compile_vertex_shader(POLARIS_SHADERS_DIR "/" #x ".hlsl"))) \
    return -1;
#define compile_pixel_shader_helper(x) \
  if (!(x##_hlsl = compile_pixel_shader(POLARIS_SHADERS_DIR "/" #x ".hlsl"))) \
    return -1;
