  ...
  return DRM_MODE_CONNECTOR_##y
    ...
    public:
      ...
      void publish_capture_metadata(platf::img_t &img, bool gpu_resident) {
        // Only a frame still on the GPU is in the format the scanout was in. The other path reads
        // back through GL as eight bit BGRA whatever the source held, so the ten bits are already
        // gone by the time anybody asks, and saying otherwise would be a lie that costs a picture.
        auto format = platf::frame_format_e::bgra8;
        if (gpu_resident) {
          if (const auto *descriptor = dynamic_cast<const egl::img_descriptor_t *>(&img)) {
            format = kms_capture::frame_format_for_fourcc(descriptor->sd.fourcc);
          }
        }
        img.frame_metadata = kms_capture::frame_metadata(gpu_resident, card.render_node, format);
    ...
    public:
      ...
      void blend_cursor(img_t &img) {
        ...
        // Compute scale factor between source capture and output image
        float scale_x = (img_width > 0) ? (float) screen_width / img_width : 1.0f;
        float scale_y = (img_height > 0) ? (float) screen_height / img_height : 1.0f;

        // Scale cursor dimensions and position
        int32_t scaled_cursor_w = (int32_t) (captured_cursor.src_w * scale_x + 0.5f);
        int32_t scaled_cursor_h = (int32_t) (captured_cursor.src_h * scale_y + 0.5f);

        // This is the position in the target that we will start drawing the cursor
        auto cursor_x = std::max<int32_t>(0, (int32_t) ((captured_cursor.x - img_offset_x) * scale_x));
        auto cursor_y = std::max<int32_t>(0, (int32_t) ((captured_cursor.y - img_offset_y) * scale_y));

        auto delta_height = std::min<int32_t>(scaled_cursor_h, std::max<int32_t>(0, screen_height - cursor_y));
        auto delta_width = std::min<int32_t>(scaled_cursor_w, std::max<int32_t>(0, screen_width - cursor_x));

        for (auto y = 0; y < delta_height; ++y) {
          // Map scaled Y back to source cursor Y using nearest-neighbor sampling
          int src_y = (int) (y / scale_y);
          if (src_y >= (int) captured_cursor.src_h) break;

          auto pixels_begin = &pixels[(y + cursor_y) * (img.row_pitch / img.pixel_pitch) + cursor_x];

          for (auto x = 0; x < delta_width; ++x) {
            // Map scaled X back to source cursor X
            int src_x = (int) (x / scale_x);
            if (src_x >= (int) captured_cursor.src_w) break;

            auto cursor_pixel = *(uint32_t *) &captured_cursor.pixels.data()[(src_y * captured_cursor.src_w + src_x) * 4];
            auto colors_in = (uint8_t *) pixels_begin;

            auto alpha = cursor_pixel >> 24u;
            if (alpha == 255) {
