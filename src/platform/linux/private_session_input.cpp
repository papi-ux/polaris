    ...
    bool ensure_generated_companion_file(
      ...
      if (fs::exists(path, ec)) {
        ...
        if (current.find(marker) == std::string::npos) {
          status_out = "keeping " + path.string() + "; it was not generated by Polaris. Delete it "
                       "to hand the file back to Polaris";
          return false;
        }
        if (current == contents) {
          status_out = path.string() + " is already current";
          return true;
        }
      }
      ...
      if (ec) {
        status_out = "could not create " + path.parent_path().string() + ": " + ec.message();
        return false;
      }
      ...
      if (ec) {
        fs::remove(tmp_path);
        status_out = "could not install " + path.string() + ": " + ec.message();
        return false;
      }

      status_out = "generated " + path.string();
      return true;
    }
  }  // namespace
