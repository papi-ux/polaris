...
namespace platf::private_session_input {
  ...
  bool ensure_generated_rc_xml(
    ...
    if (fs::exists(rc_path, ec)) {
      ...
      if (buffer.str().find(generated_marker) == std::string::npos) {
        status_out = "left the existing rc.xml alone; it was not generated by Polaris, "
                     "so host input isolation is this session's own configuration";
        return false;
      }
    }
