...
namespace platf::private_session_input {
  ...
  namespace {
    constexpr auto private_config_display_dir = "~/.config/labwc-polaris"sv;

    std::string private_config_display_path(const fs::path &path) {
      const auto filename = path.filename().string();
      if (filename.empty()) {
        return std::string {private_config_display_dir};
      }
      return std::string {private_config_display_dir} + "/" + filename;
    }
  ...
  bool ensure_generated_rc_xml(
    ...
    if (fs::exists(rc_path, ec)) {
      ...
      if (current.find(generated_marker) == std::string::npos) {
        status_out = "keeping " + rc_display_path + "; it was not generated by Polaris, so this "
      ...
      if (current == contents) {
        ...
        status_out = rc_display_path + " is already current";
        return true;
      }
    }
    ...
    if (ec) {
      status_out = "could not create " + std::string {private_config_display_dir} + ": " + ec.message();
      return false;
    }
    ...
    const auto tmp_path = rc_path.string() + ".tmp";
    {
      ...
      if (!out) {
        status_out = "could not write " + rc_display_path + ".tmp";
        return false;
      }
      out << contents;
      if (!out) {
    ...
    if (ec) {
      fs::remove(tmp_path);
      status_out = "could not install " + rc_display_path + ": " + ec.message();
      return false;
    }
  ...
  namespace {
    // Shared write path for the generated companion files (menu.xml, autostart).
    // Same ownership contract as rc.xml: a file without the marker is the
    // user's own and is never touched; writes go through a temporary file so a
    // starting labwc never reads a half-written config.
    bool ensure_generated_companion_file(
      const fs::path &path,
      const std::string &contents,
      std::string_view marker,
      std::string &status_out
    ) {
      const auto display_path = private_config_display_path(path);
      std::error_code ec;
      if (fs::exists(path, ec)) {
        ...
        if (current.find(marker) == std::string::npos) {
          status_out = "keeping " + display_path + "; it was not generated by Polaris. Delete it "
                       "to hand the file back to Polaris";
          return false;
        }
        if (current == contents) {
          status_out = display_path + " is already current";
          return true;
        }
      }
      ...
      const auto tmp_path = path.string() + ".tmp";
      {
        std::ofstream out {tmp_path, std::ios::trunc};
        if (!out) {
          status_out = "could not write " + display_path + ".tmp";
          return false;
        }
        out << contents;
        if (!out) {
      ...
      if (ec) {
        fs::remove(tmp_path);
        status_out = "could not install " + display_path + ": " + ec.message();
        return false;
      }

      status_out = "generated " + display_path;
      return true;
    }
  }  // namespace
  ...
  std::string build_menu_xml() {
    ...
    menu << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
         << generated_marker << "\n"
         << "     Put your own menu.xml here to take over: a file without the line above\n"
         << "     is left alone. -->\n"
         << "<openbox_menu>\n"
         ...
         << "    </item>\n"
         << "    <separator />\n"
         << "    <item label=\"End private session\">\n"
         << "      <action name=\"Exit\" />\n"
         << "    </item>\n"
         << "  </menu>\n"
         << "</openbox_menu>\n";
  ...
  std::string build_autostart() {
    ...
    script << generated_shell_marker << "\n"
           << "# Put your own autostart here to take over: a file without the line above\n"
           << "# is left alone.\n"
           << "# A solid background makes an empty private session read as intentional\n"
  ...
  bool ensure_generated_menu_xml(const fs::path &config_dir, std::string &status_out) {
    return ensure_generated_companion_file(config_dir / "menu.xml", build_menu_xml(), generated_marker, status_out);
  }

  bool ensure_generated_autostart(const fs::path &config_dir, std::string &status_out) {
    return ensure_generated_companion_file(config_dir / "autostart", build_autostart(), generated_shell_marker, status_out);
  }
