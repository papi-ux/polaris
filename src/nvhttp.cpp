...
#ifdef __linux__
    // A resumed isolated session (family mode) must keep stream-only audio:
    // the client's localAudioPlayMode would otherwise re-enable host audio and
    // leak the desktop's sound into the stream.
