...
struct ExternalCommandTestData {
  ...
  std::string working_directory;  // Optional: if empty, uses POLARIS_SOURCE_DIR
...
protected:
  ...
  static std::pair<int, std::string> runCommand(const std::string &cmd, const std::string_view &working_dir) {
    ...
    // Determine the working directory: use the provided working_dir or fall back to POLARIS_SOURCE_DIR
    boost::filesystem::path effective_working_dir;

    if (!working_dir.empty()) {
      effective_working_dir = working_dir;
    } else {
      // Use POLARIS_SOURCE_DIR CMake definition as the default working directory
      effective_working_dir = POLARIS_SOURCE_DIR;
    }
...
constexpr auto SIMPLE_COMMAND = IS_WINDOWS ? "where cmd" : "which sh";
  ...
  #define UDEV_TESTS \
    ExternalCommandTestData { \
      std::format("{} verify {}/src_assets/linux/misc/60-polaris.rules", UDEVADM_EXECUTABLE, POLARIS_TEST_BIN_DIR), \
      "linux", \
      true, \
