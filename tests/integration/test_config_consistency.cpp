...
protected:
  ...
  // Helper function to find and extract tabs content from the Vue config view
  static std::string extractTabsContent(const std::string &content) {
    const size_t tabsStart = content.find("const tabs = ref([");
    if (tabsStart == std::string::npos) {
      return "";
    }

    // Find the end of the tab array
    size_t pos = tabsStart + 17;  // Skip "const tabs = ref(["
    int bracketLevel = 1;
    size_t tabsEnd = pos;
    ...
    return content.substr(tabsStart + 17, tabsEnd - tabsStart - 17);
  }
  ...
  static std::map<std::string, std::string, std::less<>> extractConfigHtmlOptions() {
    ...
    const std::string content = file_handler::read_file("src_assets/common/assets/web/views/ConfigView.vue");
  ...
  // Helper function to check if an option exists in ConfigView options
...
TEST_F(ConfigConsistencyTest, AllConfigOptionsExistInAllFiles) {
  ...
  const std::set<std::string, std::less<>> internalOptions = {
    ...
    "linux_primary_output",
    "linux_streaming_output",
    "linux_use_cage_compositor",
    "max_sessions",  // Runtime stream limit, currently not exposed in config UI/docs
    "nvenc_tune",  // AI/client optimization output, not a direct host config control
