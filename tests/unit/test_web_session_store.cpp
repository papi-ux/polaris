...
TEST_F(WebSessionStoreTest, DurabilityUncertainCreatePublishesVisibleStateButFailsClosed) {
  const auto id = digest('a');
  web_session_store::manager_t manager(target, fingerprint, policy);
  ...
  EXPECT_EQ(manager.validate(id, at(1001, 1)), web_session_store::validation_status_e::io_error);

  private_state_file::set_write_fault_for_tests(private_state_file::write_fault_e::none);
  web_session_store::manager_t restarted(target, fingerprint, policy);
  ASSERT_EQ(restarted.load(at(1001, 0)), web_session_store::load_status_e::loaded);
  EXPECT_EQ(restarted.validate(id, at(1001, 1)), web_session_store::validation_status_e::valid);
}

TEST_F(WebSessionStoreTest, GuardedCreateRejectsAStaleVerifiedCredentialGeneration) {
  const auto stale_fingerprint = fingerprint;
  const auto current_fingerprint = digest('e');
  const auto id = digest('a');
  web_session_store::manager_t manager(target, stale_fingerprint, policy);
  ASSERT_TRUE(manager.rotate_credentials(current_fingerprint, at(1000, 0)));

  EXPECT_EQ(
    manager.create_for_fingerprint(id, stale_fingerprint, at(1001, 1)),
    web_session_store::creation_status_e::credential_mismatch
  );
  EXPECT_EQ(manager.size(), 0U);
  EXPECT_EQ(
    manager.create_for_fingerprint(id, current_fingerprint, at(1001, 1)),
    web_session_store::creation_status_e::created
  );
}
...
TEST_F(WebSessionStoreTest, DurabilityUncertainRevocationNeverLeavesTheOldSessionAuthorized) {
  const auto id = digest('a');
  web_session_store::manager_t manager(target, fingerprint, policy);
  ASSERT_TRUE(manager.create(id, at(1000, 0)));
  private_state_file::set_write_fault_for_tests(private_state_file::write_fault_e::post_rename_durability);
  ...
  web_session_store::manager_t restarted(target, fingerprint, policy);
  ASSERT_EQ(restarted.load(at(1001, 0)), web_session_store::load_status_e::loaded);
  EXPECT_EQ(restarted.validate(id, at(1001, 1)), web_session_store::validation_status_e::invalid);
}

TEST_F(WebSessionStoreTest, DurabilityUncertainInvalidateAllNeverLeavesAnySessionAuthorized) {
  ...
  EXPECT_FALSE(manager.invalidate_all());
  EXPECT_EQ(manager.size(), 0U);
  EXPECT_EQ(manager.validate(first, at(1002, 2)), web_session_store::validation_status_e::io_error);
  EXPECT_EQ(manager.validate(second, at(1002, 2)), web_session_store::validation_status_e::io_error);
}
...
TEST_F(WebSessionStoreTest, DurabilityUncertainTouchKeepsAuthorizationUnavailable) {
  const auto id = digest('a');
  web_session_store::manager_t manager(target, fingerprint, policy);
  ASSERT_TRUE(manager.create(id, at(1000, 0)));
  private_state_file::set_write_fault_for_tests(private_state_file::write_fault_e::post_rename_durability);

  EXPECT_EQ(manager.validate(id, at(1006, 6)), web_session_store::validation_status_e::io_error);
  private_state_file::set_write_fault_for_tests(private_state_file::write_fault_e::none);
  EXPECT_EQ(manager.validate(id, at(1007, 7)), web_session_store::validation_status_e::io_error);

  web_session_store::manager_t restarted(target, fingerprint, policy);
  ASSERT_EQ(restarted.load(at(1007, 0)), web_session_store::load_status_e::loaded);
  EXPECT_EQ(restarted.validate(id, at(1007, 1)), web_session_store::validation_status_e::valid);
}
...
TEST_F(WebSessionStoreTest, DurabilityUncertainStartupPruneRetainsVisibleStateButFailsClosed) {
  const auto expired_id = digest('a');
  const auto valid_id = digest('b');
  web_session_store::manager_t first(target, fingerprint, policy);
  ASSERT_TRUE(first.create(expired_id, at(1000, 0)));
  ...
  private_state_file::set_write_fault_for_tests(private_state_file::write_fault_e::post_rename_durability);
  web_session_store::manager_t uncertain(target, fingerprint, policy);
  EXPECT_EQ(uncertain.load(at(1020, 0)), web_session_store::load_status_e::io_error);
  EXPECT_EQ(uncertain.size(), 1U);
  EXPECT_EQ(uncertain.validate(valid_id, at(1020, 0)), web_session_store::validation_status_e::io_error);

  private_state_file::set_write_fault_for_tests(private_state_file::write_fault_e::none);
  web_session_store::manager_t restarted(target, fingerprint, policy);
  ASSERT_EQ(restarted.load(at(1020, 0)), web_session_store::load_status_e::loaded);
  EXPECT_EQ(restarted.size(), 1U);
  EXPECT_EQ(restarted.validate(valid_id, at(1020, 0)), web_session_store::validation_status_e::valid);
}
...
TEST_F(WebSessionStoreTest, DurabilityUncertainClockRollbackPublishesRevocationButFailsClosed) {
  const auto id = digest('a');
  web_session_store::manager_t manager(target, fingerprint, policy);
  ASSERT_TRUE(manager.create(id, at(1000, 0)));
  private_state_file::set_write_fault_for_tests(private_state_file::write_fault_e::directory_close);

  EXPECT_EQ(manager.validate(id, at(900, 1)), web_session_store::validation_status_e::io_error);
  EXPECT_EQ(manager.size(), 0U);
  private_state_file::set_write_fault_for_tests(private_state_file::write_fault_e::none);
  EXPECT_EQ(manager.validate(id, at(1001, 2)), web_session_store::validation_status_e::io_error);

  web_session_store::manager_t restarted(target, fingerprint, policy);
  ASSERT_EQ(restarted.load(at(1001, 0)), web_session_store::load_status_e::loaded);
  EXPECT_EQ(restarted.validate(id, at(1001, 0)), web_session_store::validation_status_e::invalid);
}
...
TEST_F(WebSessionStoreTest, DurabilityUncertainRotationPublishesNewGenerationButFailsClosed) {
  const auto old_id = digest('a');
  const auto new_fingerprint = digest('e');
  web_session_store::manager_t manager(target, fingerprint, policy);
  ASSERT_TRUE(manager.create(old_id, at(1000, 0)));
  ...
  EXPECT_EQ(manager.size(), 0U);
  EXPECT_EQ(manager.validate(old_id, at(1001, 1)), web_session_store::validation_status_e::io_error);

  private_state_file::set_write_fault_for_tests(private_state_file::write_fault_e::none);
  web_session_store::manager_t restarted(target, new_fingerprint, policy);
  ASSERT_EQ(restarted.load(at(1002, 0)), web_session_store::load_status_e::loaded);
  EXPECT_EQ(restarted.validate(old_id, at(1002, 0)), web_session_store::validation_status_e::invalid);
}
...
TEST(WebSessionCredentialLifecycleSourceTest, SerializesSaltAndCredentialTransactions) {
  ...
  EXPECT_NE(text.find("std::recursive_mutex s_credential_lifecycle_mutex;"), std::string::npos);
  constexpr std::string_view lock_statement =
    "std::lock_guard credential_lock {s_credential_lifecycle_mutex};";
  const auto expect_locked_before = [&](std::string_view function, std::string_view next_function) {
    const auto begin = text.find(function);
    const auto end = text.find(next_function, begin + function.size());
    ASSERT_NE(begin, std::string::npos) << function;
    ASSERT_NE(end, std::string::npos) << next_function;
    const auto lock = text.find(lock_statement, begin + function.size());
    EXPECT_LT(lock, end) << function;
  };
  expect_locked_before("std::string session_cookie_hash(", "std::string web_session_credential_fingerprint(");
  expect_locked_before("std::string web_session_credential_fingerprint(", "web_session_store::policy_t web_session_policy(");
  expect_locked_before("std::optional<std::string> create_web_session(", "web_session_store::validation_status_e authenticate_web_session_cookie(");
  expect_locked_before("bool authenticate(resp_https_t", "bool authenticatePolarisSession(");
  expect_locked_before("void savePassword(", "void getOTP(");
  expect_locked_before("void login(", "void logout(");
  EXPECT_NE(text.find("create_for_fingerprint("), std::string::npos);

  const auto save_password_begin = text.find("void savePassword(");
  const auto save_password_end = text.find("void getOTP(", save_password_begin);
  ASSERT_NE(save_password_begin, std::string::npos);
  ASSERT_NE(save_password_end, std::string::npos);
  const auto save_password = text.substr(save_password_begin, save_password_end - save_password_begin);
  const auto save_result = save_password.find("const auto save_result = http::save_user_creds(");
  const auto reload_result = save_password.find("const auto reload_result = http::reload_user_creds(");
  const auto disable_credentials = save_password.find("s_loaded_credentials_available = false;", reload_result);
  const auto disable_sessions = save_password.find("s_web_sessions.reset();", reload_result);
  const auto enable_credentials = save_password.find("s_loaded_credentials_available = true;", reload_result);
  EXPECT_NE(save_result, std::string::npos);
  EXPECT_NE(reload_result, std::string::npos);
  EXPECT_NE(disable_credentials, std::string::npos);
  EXPECT_NE(disable_sessions, std::string::npos);
  EXPECT_NE(enable_credentials, std::string::npos);
  EXPECT_GT(reload_result, save_result);
  EXPECT_GT(disable_credentials, reload_result);
  EXPECT_GT(disable_sessions, reload_result);
  EXPECT_GT(enable_credentials, disable_sessions);

  const auto authenticate_begin = text.find("bool authenticate(resp_https_t");
  const auto authenticate_end = text.find("bool authenticatePolarisSession(", authenticate_begin);
  ASSERT_NE(authenticate_begin, std::string::npos);
  ASSERT_NE(authenticate_end, std::string::npos);
  const auto authenticate = text.substr(authenticate_begin, authenticate_end - authenticate_begin);
  const auto authenticate_availability_gate = authenticate.find("if (!s_loaded_credentials_available) {");
  const auto bearer_authentication = authenticate.find("// Check for API key authentication via Bearer token");
  EXPECT_NE(authenticate_availability_gate, std::string::npos);
  EXPECT_NE(bearer_authentication, std::string::npos);
  EXPECT_LT(authenticate_availability_gate, bearer_authentication);

  const auto login_begin = text.find("void login(");
  const auto login_end = text.find("void logout(", login_begin);
  ASSERT_NE(login_begin, std::string::npos);
  ASSERT_NE(login_end, std::string::npos);
  const auto login = text.substr(login_begin, login_end - login_begin);
  const auto login_availability_gate = login.find("if (!s_loaded_credentials_available) {");
  const auto password_verification = login.find("http::verify_user_password(");
  EXPECT_NE(login_availability_gate, std::string::npos);
  EXPECT_NE(password_verification, std::string::npos);
  EXPECT_LT(login_availability_gate, password_verification);

  const auto upgrade_result = login.find("const auto upgrade_result = http::upgrade_user_password_hash(");
  const auto reload_failure = login.find("credential_upgrade_status_e::reload_failed", upgrade_result);
  const auto disable_after_upgrade = login.find("s_loaded_credentials_available = false;", reload_failure);
  const auto reset_after_upgrade = login.find("s_web_sessions.reset();", reload_failure);
  const auto unavailable_after_upgrade = login.find("write_session_validation_unavailable(response);", reload_failure);
  const auto save_failure = login.find("credential_upgrade_status_e::save_failed", unavailable_after_upgrade);
  EXPECT_NE(upgrade_result, std::string::npos);
  EXPECT_NE(reload_failure, std::string::npos);
  EXPECT_NE(disable_after_upgrade, std::string::npos);
  EXPECT_NE(reset_after_upgrade, std::string::npos);
  EXPECT_NE(unavailable_after_upgrade, std::string::npos);
  EXPECT_NE(save_failure, std::string::npos);
  EXPECT_LT(upgrade_result, reload_failure);
  EXPECT_LT(reload_failure, disable_after_upgrade);
  EXPECT_LT(disable_after_upgrade, reset_after_upgrade);
  EXPECT_LT(reset_after_upgrade, unavailable_after_upgrade);
  EXPECT_LT(unavailable_after_upgrade, save_failure);
}
