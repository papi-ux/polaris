...
steamos_guide_facts=(
  ...
  "Steam Deck OLED on SteamOS 3.8.16"
  "suspend and resume"
  "not certified yet"
  "Polaris-steamos3.8-x86_64.pkg.tar.zst"
...
release_notes = strip_html_comments(
    Path("docs/release-notes/v1.4.12.md").read_text(encoding="utf-8")
)
...
required_release_facts = (
    ...
    "no longer exits when it restarts its own gamescope",
...
for fact in required_release_facts:
    if fact not in current_release_prose:
        print(f"v1.4.12 changelog is missing final release fact: {fact}", file=sys.stderr)
        sys.exit(1)
...
for label, section in (("v1.4.12 changelog", current_release_prose),):
...
for label, section, expected_section_assets in (
    ...
    ("v1.4.12 changelog", current_release_prose, expected_changelog_assets),
):
...
release_notes_facts = (
    "Your Steam Deck is a streaming host now, Game Mode and all",
    "Polaris 1.4.12 is matched with Nova 1.4.12",
...
for fact in release_notes_facts:
    if fact not in release_notes:
        print(f"v1.4.12 release notes are missing release fact: {fact}", file=sys.stderr)
        sys.exit(1)
release_asset_lines = [
    line for line in release_notes.splitlines() if line.startswith("**Assets:**")
]
if len(release_asset_lines) != 1:
    print("v1.4.12 release notes must contain exactly one Assets line", file=sys.stderr)
    sys.exit(1)
release_note_assets = Counter(asset_pattern.findall(release_asset_lines[0]))
if release_note_assets != expected_assets:
    print(
        "v1.4.12 release-note Assets line must contain only the four supported packages; "
...
if release_notes.count(withdrawn_sysext_asset) != 0:
    print(
        "v1.4.12 release notes must not name the withdrawn system extension file; that warning lives in v1.4.3",
        file=sys.stderr,
    )
    sys.exit(1)
for forbidden in (
    ...
    if forbidden in current_release_prose or forbidden in release_notes:
        print(f"v1.4.12 public release scope must exclude: {forbidden}", file=sys.stderr)
        sys.exit(1)
...
if release_notes.count("systemctl --user enable --now polaris") != 1:
    print("v1.4.12 release notes must start Polaris once after SteamOS read-only restoration", file=sys.stderr)
    sys.exit(1)
