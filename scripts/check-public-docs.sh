...
release_notes = strip_html_comments(
    Path("docs/release-notes/v1.3.12.md").read_text(encoding="utf-8")
)
...
required_release_facts = (
    "Arch packaging compatibility hotfix",
    "Boost 1.91",
    "Boost 1.92",
    "libboost_locale.so=1.92.0-64",
    "current rolling Arch",
...
for fact in required_release_facts:
    if fact not in current_release_prose:
        print(f"v1.3.12 changelog is missing final release fact: {fact}", file=sys.stderr)
        sys.exit(1)
...
for label, section in (("v1.3.12 changelog", current_release_prose),):
...
for label, section in (
    ...
    ("v1.3.12 changelog", current_release_prose),
):
...
release_notes_facts = (
    ...
    "libboost_locale.so=1.92.0-64",
    "current rolling Arch",
    "no Polaris runtime-code, protocol, configuration, or Nova client change",
    "known private-Steam shutdown coredump is fixed",
...
for fact in release_notes_facts:
    if fact not in release_notes:
        print(f"v1.3.12 release notes are missing release fact: {fact}", file=sys.stderr)
        sys.exit(1)
if release_notes.count("sudo -H polaris --setup-host &&") != 3:
    print("v1.3.12 release notes must chain setup-host in all three mutable package commands", file=sys.stderr)
    sys.exit(1)
if release_notes.count("systemctl --user restart polaris") != 3:
    print("v1.3.12 release notes must restart Polaris in all three mutable package commands", file=sys.stderr)
    sys.exit(1)
