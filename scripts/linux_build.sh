...
publisher_website=""
publisher_issue_url="https://github.com/papi-ux/polaris/issues"
skip_cleanup=0
skip_cuda=0
skip_libva=0
skip_package=0
...
function install_cuda() {
  ...
  if [ ! -f "${build_dir}/cuda/bin/nvcc" ]; then
    echo "cuda url: ${url}"
    wget "$url" -c --progress=bar:force:noscroll -q --show-progress -O "${build_dir}/cuda.run"
    chmod a+x "${build_dir}/cuda.run"
    mkdir -p "${build_dir}/tmp"
    "${build_dir}/cuda.run" --silent --toolkit --toolkitpath="${build_dir}/cuda" --no-opengl-libs --no-man-page --no-drm --tmpdir="${build_dir}/tmp" --override
    rm "${build_dir}/cuda.run"
  else
    echo "CUDA already installed found at ${build_dir}/cuda/bin/nvcc"
  fi

  # Patch CUDA math_functions.h for Fedora glibc compatibility (noexcept mismatch)
  local math_h="${build_dir}/cuda/targets/x86_64-linux/include/crt/math_functions.h"
  if [ -f "$math_h" ]; then
    echo "Patching CUDA math_functions.h for noexcept compatibility..."
    sed -i -E '/__THROW/!s/(extern __DEVICE_FUNCTIONS_DECL__ __device_builtin__ .+ (cospi|sinpi|cospif|sinpif|rsqrt|rsqrtf)\(.+\));/\1 __THROW;/g' "$math_h"
  fi
...
function run_step_deps() {
  ...
  if ! check_version "cmake" "$cmake_min" "inf"; then
    ...
    echo "cmake url: ${url}"
    wget "$url" -c --progress=bar:force:noscroll -q --show-progress -O "${build_dir}/cmake.sh"
  ...
  if ! check_version "doxygen" "$doxygen_min" "$doxygen_max"; then
    if [ "${POLARIS_COMPILE_DOXYGEN}" == "true" ]; then
      ...
      pushd "${build_dir}"
        wget "$doxygen_url" -c --progress=bar:force:noscroll -q --show-progress -O "doxygen.tar.gz"
...
function run_step_cmake() {
  ...
  cmake_args=(
    ...
    "-DPOLARIS_ENABLE_NATIVE_ARCH=ON"
  )

  if [ "$wayland_only" == 1 ]; then
    cmake_args+=("-DPOLARIS_ENABLE_X11=OFF")
  fi

  if [ "$appimage_build" == 1 ]; then
    cmake_args+=("-DPOLARIS_BUILD_APPIMAGE=ON")
  fi

  # Publisher metadata
  if [ -n "$publisher_name" ]; then
    cmake_args+=("-DPOLARIS_PUBLISHER_NAME='${publisher_name}'")
  fi
  if [ -n "$publisher_website" ]; then
    cmake_args+=("-DPOLARIS_PUBLISHER_WEBSITE='${publisher_website}'")
  fi
  if [ -n "$publisher_issue_url" ]; then
    cmake_args+=("-DPOLARIS_PUBLISHER_ISSUE_URL='${publisher_issue_url}'")
  fi
...
if grep -q "Arch Linux" /etc/os-release; then
  ...
  nvm_node=0
  dev_tools_group="development-tools"
elif grep -q "PLATFORM_ID=\"platform:f43\"" /etc/os-release || (grep -q 'ID=fedora' /etc/os-release && grep -q 'VERSION_ID=43' /etc/os-release); then
  distro="fedora"
  version="43"
  package_update_command="${sudo_cmd} dnf update -y"
  package_install_command="${sudo_cmd} dnf install -y"
...
# Resolve absolute path without .. to satisfy wget2
