...
- Grades encoder time against the active stream's real FPS budget, avoids recommending a GPU-native mode that already fell back, and points an AMD Private Stream explicitly pinned to VA-API toward Auto's Vulkan live probe with VA-API fallback
