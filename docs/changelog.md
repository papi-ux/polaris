...
- Says so when client keyboard and mouse seat isolation cannot take effect: Polaris reads back the marker on each virtual keyboard, mouse, touch and pen it creates and warns when the host's input backend dropped it, instead of leaving a setting that saves, reads back as enabled, and silently leaves the devices on the desktop session's seat
- Carries the platform and runtime of imported Heroic titles through the scan and the import response, so a Windows title running under Proton is no longer indistinguishable from a Linux native build until it launches; the platform, the runtime, and the runtime's own label are reported or left blank, never guessed
