...
- Binds optimize results to the canonical app UUID or ID and exact host-default, mirror, or private topology choice, and revalidates the complete deterministic profile before both fresh launch and resume
...
- Restricts AI and Codex CLI output to typed explanatory text that cannot define settings, actions, confidence, or launch policy; subscription `deterministic-fallback` remains informational and quiet
