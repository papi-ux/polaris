...
if "%~1"=="" (
    ...
    echo   add    - Adds Polaris directories to system PATH
    echo   remove - Removes Polaris directories from system PATH
    exit /b 1
)

rem Get Polaris root directory
for %%I in ("%~dp0\..") do set "ROOT_DIR=%%~fI"
echo Polaris root directory: !ROOT_DIR!
...
if /i "%~1"=="add" (
    ...
    if "!NEW_PATH!" neq "!CURRENT_PATH!" (
        ...
        if !ERRORLEVEL!==0 (
            echo Successfully added Polaris directories to PATH
        ) else (
            echo Failed to add Polaris directories to PATH
        )
    ) else (
...
if /i "%~1"=="remove" (
    ...
    if "!CHANGES_MADE!"=="1" (
        ...
        if !ERRORLEVEL!==0 (
            echo Successfully removed Polaris directories from PATH
        ) else (
            echo Failed to remove Polaris directories from PATH
        )
    ) else (
        echo No changes needed to PATH
    )
