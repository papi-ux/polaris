...
rem Get polaris root directory
for %%I in ("%~dp0\..") do set "ROOT_DIR=%%~fI"

set SERVICE_NAME=PolarisService
set "SERVICE_BIN=%ROOT_DIR%\tools\polarissvc.exe"
set "SERVICE_CONFIG_DIR=%LOCALAPPDATA%\SudoMaker\Polaris"
...
rem Remove the legacy PolarisSvc service
net stop polarissvc
sc delete polarissvc

rem Check if PolarisService already exists
...
sc %SC_CMD% %SERVICE_NAME% binPath= "\"%SERVICE_BIN%\"" start= %SERVICE_START_TYPE% DisplayName= "Polaris Service"

rem Set the description of the service
sc description %SERVICE_NAME% "Polaris is a self-hosted game stream host for Moonlight."
